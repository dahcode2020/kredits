...
- **`tSiCle` résout désormais la forme pointée** (`banque.chat.x`) comme la forme deux-points : le
  chat et les motifs de transaction stockés en clé s'affichent traduits ×4, le texte libre passe
  tel quel (verrou `tests/unit/tsicle.spec.ts`).
...
- **Documents justificatifs** (`/api/documents` + `/api/notifications`, tables `documents` et
  `notifications`) : le client téléverse ses pièces dans le menu Documents (fichier réel, plafonné
  à 5 Mo côté client ET serveur, codes du moteur uniquement, une pièce approuvée est figée) ;
...
- **Compte admin : le contrat devient un « LOAN AGREEMENT » complet.** Le menu Contrats intègre le
  ...
  null = retour au défaut — 17 nouveaux (235 au total).
...
- **Compte admin : un vrai menu « Documents » transversal.** À la place du menu client, le
  personnel voit TOUTES les pièces déposées par TOUS les clients (même table du magasin, mêmes
  codes `credit:documents.*`, mêmes statuts) : filtres par client et par statut, compteurs
  « à approuver / approuvées », lecture et téléchargement de la pièce, approbation en un geste
...
- **Virement sortant : IBAN de n'importe quel pays.** La validation accepte désormais tout IBAN du
  ...
  (verrous : 9 IBAN pays acceptés, checksum/longueur/pays inconnu refusés, virement FR initié).
- **Champs lisibles, fond clair garanti.** `colorScheme` forcé sur `light` (le thème sombre de l'OS
  rendait les contrôles natifs sombres et illisibles), fond blanc + texte sombre explicites sur
  tous les champs, autofill neutralisé (`-webkit-box-shadow` blanc).
- **Espace client, menu Demandes : aperçu des demandes en cours de chaque client.** Les demandes
  vivent désormais dans la table du magasin (`/api/demandes`, chaque client ne voit que les
  siennes) ; le client démo reçoit deux demandes semées, l'aperçu affiche produit, montant,
  mensualité, TAEG, total, taux d'endettement, score et date ; un dépôt connecté part aussi côté
  serveur, et la règle « une demande en cours par catégorie » se vérifie serveur + appareil
  (verrous : semis, isolation par compte, dépôt).
